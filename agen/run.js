...
const MISSIONS = {
  ...
  "combo-build-shipping": {
    goal: "Bangun & kirim project produksi lintas platform A-Z: scaffold arsitektur (multi-module Android/Gradle/Compose Multiplatform, web frontend+backend monorepo), manajemen dependensi version catalog, build/bundling, Docker/containerization, manifest GKE/K8s, dan pipeline CI/CD — semua file ditulis ke workspace via tool write agar bisa diunduh & di-build.",
    steps: [
      "Tentukan target build & arsitektur dari permintaan user: (a) mobile/Android (APK/Compose Multiplatform/React Native) atau (b) web fullstack (frontend+backend) — serta skala (monolith, multi-module, monorepo).",
      "Tulis STRUKTUR & TOOLING LENGKAP memakai tool write (satu file per panggilan write). Untuk Android multi-module: settings.gradle(.kts) + plugins, gradle/libs.versions.toml (version catalog), gradle-wrapper.properties, build.gradle root, gradle.properties, modul ':app', ':core:designsystem', ':core:network', ':feature:<x>' (tiap modul build.gradle + manifest + source set). Untuk web fullstack: package.json (workspaces), konfigurasi build (vite/next), tsconfig, frontend (App/components/styling) + backend (API routes, schemas, db).",
      "Konfigurasi BUILD PRODUKSI: buildTypes & flavors/variants (debug/release, staging/prod), minify/shrink & ProGuard/R8, splits ABI (Android), environment & secret injection (env vars), serta bundle/optimasi ukuran (tree-shaking, code-split).",
      "Containerization & deploy: tulis Dockerfile multi-stage (build + runtime minimal), .dockerignore, docker-compose.yml (opsional), dan manifest GKE/K8s (deployment.yaml + service.yaml/Ingress) agar artefak siap disebar ke cloud.",
      "Pipeline CI/CD: tulis .github/workflows/ci.yml (install deps, lint, test, build artifacts) dan file release/config lainnya agar build berulang & otomatis.",
      "Eksekusi nyata bila lingkungan memungkinkan: tool bash/npm/run untuk instal & build bila package tersedia, verifikasi struktur via ls, lint sintaks. Pada serverless tanpa toolchain: pastikan semua file lengkap & konsisten, lalu beri INSTRUKSI BUILD eksplisit.",
      "Sajikan INFO BUILD & SHIPPING: daftar file, struktur folder, perintah build per target (./gradlew assembleDebug / npm run build), langkah build image & push registry, deploy ke GKE/Cloud Run, dan cara instal APK ke device.",
      "Tutup dengan [SELESAI] + ringkasan arsitektur & tautan untuk unduh project ZIP.",
    ],
  },
  "combo-build-apk": {
    goal: "Bangun APK Android kelas produksi sampai menjadi FILE APK ASLI yang siap di-install: project Gradle lengkap (version catalog, clean architecture MVVM, AGP/Kotlin/Compose, buildTypes debug+release dengan signing & R8/ProGuard, ABI splits, unit & instrumented test, lint) yang di-commit ke folder android/ sehingga pipeline GitHub Actions (.github/workflows/android.yml) mengkompilasinya menjadi app-release.apk signed & installable (artifact + release) — bukan hanya source.",
    steps: [
      "Tentukan SPESIFIKASI build dari permintaan user: jenis aplikasi (mis. editor foto/video), namespace & applicationId (mis. com.namamu.photoeditor), compileSdk/targetSdk/minSdk, dan versi tooling stabil (Gradle ~8.x, AGP ~8.x, Kotlin ~2.x, Compose BOM terbaru).",
      "Tulis TOOLING & KONFIGURASI ROOT lengkap via tool write:\n   settings.gradle(.kts) (pluginManagement + dependencyResolutionManagement repositories google/mavenCentral, rootProject.name, include ':app'),\n   gradle/libs.versions.toml (version catalog: agp, kotlin, compose-bom, lifecycle, nav, coil, coroutines; + [libraries] dan [plugins] terpusat),\n   gradle/wrapper/gradle-wrapper.properties,\n   build.gradle(.kts) root (plugins alias apply false),\n   gradle.properties (org.gradle.jvmargs=-Xmx2g, android.useAndroidX=true, android.nonTransitiveRClass=true, kotlin.code.style=official),\n   local.properties (sdk.dir placeholder).",
      "Tulis MODUL APP build.gradle(.kts) KELAS PRODUKSI: plugins (android.application, kotlin-android, compose), namespace/applicationId, compileSdk/minSdk/targetSdk, buildFeatures.compose=true & composeOptions.kotlinCompilerExtensionVersion, defaultConfig (versionCode/versionName, testInstrumentationRunner), buildTypes debug (debuggable, applicationIdSuffix '.debug') & release (minifyEnabled=true, isMinifyEnabled, shrinkResources, proguardFiles + proguard-rules.pro, signingConfig) menggunakan version catalog untuk semua dependency (Compose BOM, ui, material3, ui-tooling-preview, lifecycle-runtime-compose, activity-compose, coil, coroutines, navigation-compose, uiautomator/espresso utk test), dan splits { abi { enable; reset; include 'arm64-v8a','armeabi-v7a','x86','x86_64'; universalApk } }.",
      "Tulis KODE APLIKASI + RESOURCE definitif:\n   app/src/main/AndroidManifest.xml (application android:name, icon/label/theme, MainActivity exported + intent-filter MAIN/LAUNCHER, permissions READ/WRITE MediaStore versi API, usesCleartextTraffic=false, tools:targetApi),\n   app/src/main/java/<paket>/MainActivity.kt & UI Compose (state hoisting, Material3 theme file Theme.kt, Type.kt, Color.kt),\n   layer clean architecture MVVM (data/domain/ui + ViewModel + repository + use case) bila relevan,\n   app/src/main/res/values/strings.xml, themes.xml, colors.xml, dimens, mipmap/adaptive icon (ic_launcher), xml/backup_rules & data_extraction_rules,\n   app/src/main/res/proguard-rules.pro (keep rules utk model/serialisasi/coroutine), dan unit test (app/src/test) + androidTest instrumented.",
      "Implementasikan fungsionalitas EDITOR sesuai permintaan (pilih gambar/video via Photo Picker/ActivityResult, crop/rotate/filter bitmap/CameraX, pratinjau live, simpan ke MediaStore & bagikan) dengan pola idiomatis dan optimasi performa/keamanan (input validation, immutable state, lifecycle-aware).",
      "Eksekusi & verifikasi bila lingkungan memungkinkan: jalankan tool bash untuk ls struktur & verifikasi kelengkapan file. Karena serverless TIDAK punya Android SDK/Gradle, PROSES KOMPILASI APK ASLI dilakukan oleh pipeline CI GitHub Actions (gratis) yang sudah ada di repo: .github/workflows/android.yml menjalankan JDK17 + Gradle8 -> gradle assembleRelease dengan keystore otomatis -> menghasilkan android/app/build/outputs/apk/release/app-release.apk yang signed & bisa DI-INSTALL (duh + apksigner verify).",
      "Sajikan CARA MENDAPATKAN FILE APK ASLI yang siap di-install: (1) project ditulis ke folder android/ dan di-push ke repo; (2) workflow .github/workflows/android.yml memicu assembleRelease otomatis; (3) file app-release.apk (signed) muncul di tab Actions > Artifacts dan di GitHub Release 'APK Agen GCP #N'; (4) user mengunduh .apk lalu install langsung ke Android (izinkan 'unknown sources' bila perlu). Juga sertakan opsi lokal: ./gradlew assembleRelease -> app/build/outputs/apk/release/app-release.apk, dan adb install -r <file.apk>.",
      "Tutup dengan [SELESAI] + ringkasan: project Gradle lengkap & siap-build, jenis file APK yang akan dihasilkan (app-release.apk signed & installable), dan tautan untuk mengunduh hasil: ZIP source (download project) + GitHub Actions Artifacts/Release berisi file .apk asli yang bisa di-install di Android.",
    ],
  },
