...
const MISSIONS = {
  ...
  "combo-build-apk": {
    goal: "Bangun APK Android kelas produksi A-Z dengan standar profesional: project Gradle lengkap dengan version catalog, clean architecture MVVM, tooling modern (AGP/Kotlin/Compose), buildTypes debug+release dengan signing & R8/ProGuard, ABI splits, unit & instrumented test, lint, dan pipeline CI/CD — setiap file ditulis ke workspace via tool write agar source bisa diunduh lalu di-build di Android Studio / gradle.",
    steps: [
      ...
      "Eksekusi & verifikasi bila lingkungan memungkinkan: jalankan tool bash untuk ls struktur, verifikasi kelengkapan file, dan lint sintaks bila Java/Kotlin tersedia; jika serverless TANPA Android SDK, pastikan semua file lengkap & konsisten lalu beri INSTRUKSI BUILD eksplisit.",
      "Sajikan INSTRUKSI BUILD & DISTRIBUSI APK profesional: (shell) ./gradlew assembleDebug -> app/build/outputs/apk/debug/app-debug.apk; untuk release pastikan keystore & signingConfig lalu ./gradlew assembleRelease (sign) & ./gradlew bundleRelease -> AAB; jalankan lint ./gradlew lint dan test ./gradlew testDebugUnitTest; install ke device via adb install -r ...; dan opsional pipeline GitHub Actions (.github/workflows/build.yml) utk assemble APK otomatis di CI.",
      "Tutup dengan [SELESAI] + ringkasan arsitektur build APK & tautan untuk mengunduh project ZIP (source siap-build + CI).",
    ],
  },
